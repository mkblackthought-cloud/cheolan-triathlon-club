...
function nav(active) {
  ...
  if (state.profile?.role === 'admin') items.push(['admin', '⚙', '관리'], ['teamadmin', '♜', '팀 관리'], ['approvals', '✓', '가입승인']);
...
function mePage() { nav('me'); const month = new Date().toISOString().slice(0, 7); const scores = summaries(monthRecords()); const rows = state.athletes.map((p) => ({ profile: p, score: scores[p.id] || { total: 0 } })).sort((a, b) => b.score.total - a.score.total); main.innerHTML = `<section class="page"><div class="hero"><h1>${month} 개인 점수</h1><p>이번 달 가입 회원 전체의 점수입니다. 하루 운동 기본점수는 최초 1건만, 최대 1점입니다.</p></div><div class="card"><div class="table-wrap"><table><thead><tr><th>순위</th><th>이름</th><th>점수</th></tr></thead><tbody>${rows.map((r, i) => `<tr><td>${i + 1}</td><td>${esc(r.profile.display_name)}</td><td><b>${n(r.score.total)}점</b></td></tr>`).join('') || '<tr><td colspan="3">가입 회원이 없습니다.</td></tr>'}</tbody></table></div></div></section>`; }
function teamPage() {
...
function adminPage() {
  ...
  const teamAdminLink = document.createElement('a'); teamAdminLink.href = '#teamadmin'; teamAdminLink.className = 'btn orange full'; teamAdminLink.textContent = '팀 생성 · 배정 · 팀장 관리'; main.querySelector('.hero').insertAdjacentElement('afterend', teamAdminLink);
...
function teamAdminPage() {
  ...
  $('#members-form').onsubmit = async (e) => { e.preventDefault(); const checked = [...new FormData(e.target).getAll('member')]; if (checked.length > 4) return toast('팀원은 최대 4명입니다.'); const removed = state.athletes.filter((p) => p.team_id === selectedId && !checked.includes(p.id)); const added = state.athletes.filter((p) => checked.includes(p.id) && p.team_id !== selectedId); const results = await Promise.all(removed.map((p) => supabase.from('profiles').update({ team_id: null }).eq('id', p.id))); const removalError = results.find((r) => r.error)?.error; if (removalError) return toast(removalError.message); const addedResults = await Promise.all(added.map((p) => supabase.from('profiles').update({ team_id: selectedId }).eq('id', p.id))); const addError = addedResults.find((r) => r.error)?.error; if (addError) return toast(addError.message); await loadData(); toast('팀원 배정을 저장했습니다.'); teamAdminPage(); };
...
async function route() {
  ...
  try { await loadData(); topbar(); if (state.profile.role !== 'admin' && !state.profile.is_approved) return pendingPage(); const page = location.hash.slice(1) || 'record'; if (['admin', 'teamadmin', 'approvals'].includes(page) && state.profile.role !== 'admin') return recordPage(); ({ record: recordPage, me: mePage, team: teamPage, admin: adminPage, teamadmin: teamAdminPage, approvals: approvalsPage }[page] || recordPage)(); } catch (error) { main.innerHTML = `<section class="page"><div class="card"><h2>접속 오류</h2><p>${esc(error.message)}</p><button class="btn full" id="retry">다시 시도</button></div></section>`; $('#retry').onclick = route; }
}

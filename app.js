...
const isDormantProfile = (profile) => Boolean(profile?.is_dormant);
function competitionRanks(rows, valueOf) {
  let previousValue;
  let rank = 0;
...
function signupPage() {
  main.innerHTML = `<section class="login"><a class="brand" href="#"><img src="./cheoncheol-logo.png" alt="천철 철인클럽 로고" style="width:42px;height:42px;border-radius:50%;object-fit:cover"><strong>철안철인클럽</strong></a><div class="card"><h2>회원가입</h2><form id="signup-form"><div class="field"><label>이름</label><input name="name" required></div><div class="field"><label>아이디 (영문·숫자·_ 3~20자)</label><input name="id" pattern="[A-Za-z0-9_]{3,20}" autocomplete="username" required></div><div class="field"><label>비밀번호 (4자 이상)</label><input name="password" type="password" minlength="4" autocomplete="new-password" required></div><button class="btn orange full">가입 신청</button></form><p class="hint">가입 후 관리자의 승인 전까지는 기록을 입력할 수 없습니다.</p></div></section>`;
  $('#signup-form').onsubmit = async (e) => { e.preventDefault(); const d = new FormData(e.target); const displayName = String(d.get('name')).trim(); const existing = await supabase.from('profiles').select('*').eq('display_name', displayName); if (!existing.error && (existing.data || []).some((profile) => isDormantProfile(profile))) return toast('휴면 처리된 동일 이름은 다시 가입 신청할 수 없습니다.'); const username = String(d.get('id')).trim().toLowerCase(); const { error } = await supabase.auth.signUp({ email: accountEmail(username), password: authPassword(d.get('password')), options: { data: { display_name: displayName, username } } }); if (error) toast(`가입 실패: ${error.message}`); else toast('가입 신청이 완료되었습니다. 관리자 승인 후 이용할 수 있습니다.'); };
}
...
function mePage() {
  ...
  document.querySelectorAll('.member-record-open').forEach((button) => { button.onclick = () => { const profile = visibleAthletes.find((athlete) => athlete.id === button.dataset.id); if (profile) openMemberRecords(profile, selected.records, selected.label); }; });
...
function teamPage() {
  const month = new Date().toISOString().slice(0, 7); nav('team'); const records = monthRecords(); const scores = teamSummaries(records); const rows = competitionRanks(state.teams.map((team) => { const people = state.athletes.filter((p) => p.team_id === team.id); const leader = state.athletes.find((p) => p.id === team.leader_id); return { team, people, leader, total: n(scores[team.id]?.total) }; }).sort((a, b) => b.total - a.total), (row) => row.total); const mine = state.teams.find((t) => t.leader_id === state.user.id);

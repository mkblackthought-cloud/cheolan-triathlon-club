...
function mePage() { nav('me'); const selected = personalScorePeriod(); const scores = summaries(selected.records); const amounts = Object.fromEntries(state.athletes.map((p) => [p.id, { swim: 0, cycle: 0, run: 0 }])); selected.records.forEach((r) => { if (amounts[r.user_id]?.[r.exercise_type] !== undefined) amounts[r.user_id][r.exercise_type] += n(r.amount); }); const rows = state.athletes.map((p) => ({ profile: p, score: scores[p.id] || { total: 0 }, amounts: amounts[p.id] || { swim: 0, cycle: 0, run: 0 } })).sort((a, b) => b.score.total - a.score.total); const range = state.personalRange || {}; main.innerHTML = `<section class="page"><div class="hero"><h1>${selected.label} 개인 점수</h1><p>가입 회원 전체의 운동량과 개인 기본점수 누계입니다. 하루 기본점수는 최초 1건만, 최대 1점입니다.</p></div><div class="tabs"><button class="tab period-filter ${selected.period === 'today' ? 'active' : ''}" data-period="today">오늘</button><button class="tab period-filter ${selected.period === 'week' ? 'active' : ''}" data-period="week">이번 주</button><button class="tab period-filter ${selected.period === 'month' ? 'active' : ''}" data-period="month">이번 달</button><button class="tab period-filter ${selected.period === 'range' ? 'active' : ''}" data-period="range">기간 조회</button></div>${selected.period === 'range' ? `<div class="card"><form id="personal-range-form"><div class="grid"><div class="field"><label>시작일</label><input name="start" type="date" value="${range.start || selected.start}" required></div><div class="field"><label>종료일</label><input name="end" type="date" value="${range.end || selected.end}" required></div></div><button class="btn full">조회</button></form></div>` : ''}<div class="card"><div class="table-wrap"><table><thead><tr><th>순위</th><th>이름</th><th>수영(m)</th><th>사이클(km)</th><th>러닝(km)</th><th>점수</th></tr></thead><tbody>${rows.map((r, i) => `<tr><td>${i + 1}</td><td>${esc(r.profile.display_name)}</td><td>${n(r.amounts.swim)}</td><td>${n(r.amounts.cycle)}</td><td>${n(r.amounts.run)}</td><td><b>${n(r.score.total)}점</b></td></tr>`).join('') || '<tr><td colspan="6">가입 회원이 없습니다.</td></tr>'}</tbody></table></div></div></section>`; document.querySelectorAll('.period-filter').forEach((button) => { button.onclick = () => { state.personalPeriod = button.dataset.period; mePage(); }; }); const form = $('#personal-range-form'); if (form) form.onsubmit = (e) => { e.preventDefault(); const data = new FormData(form); if (data.get('start') > data.get('end')) return toast('시작일은 종료일보다 앞서야 합니다.'); state.personalRange = { start: data.get('start'), end: data.get('end') }; mePage(); }; }
